...
- Phase 5：状态链。候选状态按 §8.3 语义合并进上一份快照，程序计算状态增量并落成状态节点，每 N 个节点一个 Checkpoint，重放 = 最近 Checkpoint + 后续增量并校验指纹；楼 N 必须基于楼 N-1 的有效节点分析，前楼无效则任务失败而不是凭空分析；重算得到相同状态或存在相同依赖的候选时直接复用、不调用模型；`/state/current`、`/state/at` 返回当前 / 历史快照（`src/state/diff.ts`、`src/state/apply.ts`、`src/state/chain-engine.ts`、`src/storage/state-chain-store.ts`）

尚未实现：失效标记与重建队列（Phase 6）、生成前闸门（Phase 7）及之后阶段；「跟随 SillyTavern」渠道模式延后。当前 `/generation/prepare` 恒返回 ready 且注入内容为空。
