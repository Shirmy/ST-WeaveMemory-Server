...
export const TOKEN_PACKER_DEFAULTS = {
  maxMemoryCount: 6,
  fixedRecentCount: 2,
  minimumTokens: 2000,
  maximumTokens: 6000,
...
export type PackedMemory = FusedMemory & {
  priority: 'fixed_recent' | 'high_relevance' | 'other';
...
/** Conservative, dependency-free estimate. It deliberately rounds up so a pack never grows past its budget. */
...
function memoryText(item: FusedMemory): string {
  ...
  const title = memory.title?.trim() ? `${memory.title.trim()}\n` : '';
  return `[记忆 ${memory.startFloor}-${memory.endFloor}]\n${title}${memory.summary.trim()}`;
}
...
/** Packs fixed recent memories first, then ranked candidates, while counting the rendered prompt itself. */
export function packMemories(candidates: FusedMemory[], options: TokenPackOptions): TokenPackResult {
  ...
  for (const item of ordered) {
    ...
    const nextText = promptText([...packed, candidate]);
    if (estimateTokens(nextText) > limit) {
  ...
  const currentStateTokens = estimateTokens(options.currentState ?? '');
  return {
    memories: packed,
    text,
    tokenLimit: limit,
