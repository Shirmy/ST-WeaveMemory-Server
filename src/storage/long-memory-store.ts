...
type Row = { memory_id: string; chat_id: string; branch_id: string; batch_id: string; slice_id: string; start_floor: number; end_floor: number; title: string | null; summary: string; narrative_time: string | null; end_state_node_id: string; end_state_fingerprint: string; batch_dependency_fingerprint: string; source_floor_ids: string; bm25_indexed: number; embedding_indexed: number; stale: number; created_at: string; updated_at: string };

function record(row: Row, tags: string[], characterIds: string[], plotlineIds: string[]): LongMemoryRecord {
  return { memoryId: row.memory_id, chatId: row.chat_id, branchId: row.branch_id, batchId: row.batch_id, sliceId: row.slice_id, startFloor: row.start_floor, endFloor: row.end_floor, ...(row.title ? { title: row.title } : {}), summary: row.summary, tags, characterIds, plotlineIds, ...(row.narrative_time ? { narrativeTime: row.narrative_time } : {}), sourceFloorIds: JSON.parse(row.source_floor_ids || '[]') as string[], batchDependencyFingerprint: row.batch_dependency_fingerprint, endStateNodeId: row.end_state_node_id, endStateFingerprint: row.end_state_fingerprint, bm25Indexed: row.bm25_indexed === 1, embeddingIndexed: row.embedding_indexed === 1, stale: row.stale === 1, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export class LongMemoryStore {
  ...
  async findByDependency(chatId: string, branchId: string, dependency: string): Promise<LongMemoryRecord[]> {
    const records = await this.list(chatId, branchId, false);
