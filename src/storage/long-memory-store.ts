...
export class MemoryManagementError extends Error {
  readonly code = 'WM_INVALID_REQUEST';
}
...
export class LongMemoryStore {
  ...
  async activateBatch(batchId: string): Promise<void> {
    const now = new Date().toISOString();
    await this.database.transaction(async () => { const target = await this.database.get<BatchRow>('SELECT * FROM long_memory_batches WHERE batch_id = ?', [batchId]); if (!target) throw new Error(`long memory batch ${batchId} does not exist`); await this.database.run('UPDATE long_memory_batches SET stale = 1, updated_at = ? WHERE chat_id = ? AND branch_id = ? AND batch_start_floor = ? AND batch_end_floor = ? AND batch_id != ?', [now, target.chat_id, target.branch_id, target.batch_start_floor, target.batch_end_floor, batchId]); await this.database.run('UPDATE long_memories SET stale = 1, bm25_indexed = 0, embedding_indexed = 0, updated_at = ? WHERE chat_id = ? AND branch_id = ? AND batch_start_floor = ? AND batch_end_floor = ? AND batch_id != ?', [now, target.chat_id, target.branch_id, target.batch_start_floor, target.batch_end_floor, batchId]); await this.database.run('UPDATE long_memory_batches SET stale = 0, updated_at = ? WHERE batch_id = ?', [now, batchId]); await this.database.run('UPDATE long_memories SET stale = 0, bm25_indexed = 0, embedding_indexed = 0, updated_at = ? WHERE batch_id = ?', [now, batchId]); });
  }

  async setMemoryStale(memoryId: string, stale: boolean): Promise<void> {
    const target = await this.database.get<{ stale: number }>('SELECT b.stale FROM long_memories m JOIN long_memory_batches b ON b.batch_id = m.batch_id WHERE m.memory_id = ?', [memoryId]);
    if (!target) throw new MemoryManagementError('memory does not exist');
    if (!stale && target.stale) throw new MemoryManagementError('这条记忆的来源或 Prompt 已失效，请先重新总结该批次。');
    const now = new Date().toISOString();
    await this.database.run('UPDATE long_memories SET stale = ?, bm25_indexed = CASE WHEN ? = 1 THEN 0 ELSE bm25_indexed END, embedding_indexed = CASE WHEN ? = 1 THEN 0 ELSE embedding_indexed END, updated_at = ? WHERE memory_id = ?', [stale ? 1 : 0, stale ? 1 : 0, stale ? 1 : 0, now, memoryId]);
  }

  async markAllBatchesStale(chatId: string, branchId: string): Promise<void> {
    const now = new Date().toISOString();
    await this.database.transaction(async () => { await this.database.run('UPDATE long_memory_batches SET stale = 1, updated_at = ? WHERE chat_id = ? AND branch_id = ? AND stale = 0', [now, chatId, branchId]); await this.database.run('UPDATE long_memories SET stale = 1, bm25_indexed = 0, embedding_indexed = 0, updated_at = ? WHERE chat_id = ? AND branch_id = ? AND stale = 0', [now, chatId, branchId]); });
  }
