...
const equivalent = resolveExternalMappings({ ...base, mappings: [{ id: 'location', source: 'mvu', externalPath: 'role.location', weaveTarget: { domain: 'trace', characterId: 'alice', path: 'currentSituations.location' }, mode: 'equivalent', enabled: true }] });
assert.deepEqual(equivalent.activeEquivalentMappings, ['location']);
assert.deepEqual(equivalent.suppressedWeaveFields, [{ domain: 'trace', characterId: 'alice', field: 'currentSituations', semanticKey: 'location' }]);
...
const related = resolveExternalMappings({ ...base, mappings: [{ id: 'affinity', source: 'mvu', externalPath: 'role.affinity', weaveTarget: { domain: 'trace', characterId: 'alice', path: 'affinity' }, mode: 'related', enabled: true }] });
