...
function memory(memoryId: string, startFloor: number, endFloor: number, summary: string): FusedMemory {
  return { memory: { memoryId, chatId: 'chat', branchId: 'branch', batchId: 'batch', sliceId: memoryId, startFloor, endFloor, summary, tags: [], characterIds: [], plotlineIds: [], endStateNodeId: 'node', endStateFingerprint: 'fp', bm25Indexed: true, embeddingIndexed: true, stale: false, createdAt: '2026-01-01', updatedAt: '2026-01-01', sourceFloorIds: [], batchDependencyFingerprint: 'dep', batchStartFloor: startFloor, batchEndFloor: endFloor } as LongMemoryRecord, rrfScore: 1, ranks: { bm25: 1 }, scores: { bm25: 1 } };
}

const candidates = [memory('old', 1, 2, 'old event'), memory('middle', 5, 6, 'middle event'), memory('recent', 10, 12, 'recent event')];
