...
const scheduler = new LongMemoryScheduler({
  ...
  memories: { listBatches: async () => [], markAllBatchesStale: async () => {} } as never,
...
const untouched = new LongMemoryScheduler({ getSummaryIntervalFloors: async () => 50, store: { getFloor: async (floorId: string) => ({ floorKey: floorId, content: floorId }) } as never, chain: { listNodes: async () => nodes, getDeltasByNodeIds: async () => new Map() } as never, memories: { listBatches: async () => [], markAllBatchesStale: async () => {} } as never, generator: { generate: async () => { throw new Error('must not trigger'); } } as never });
  await untouched.onStateCommitted({ chatId: 'chat', branchId: 'branch' });
  let staleRegenerated = 0;
  const staleScheduler = new LongMemoryScheduler({ getSummaryIntervalFloors: async () => 30, store: { getFloor: async (floorId: string) => ({ floorKey: floorId, content: floorId }) } as never, chain: { listNodes: async () => nodes, getDeltasByNodeIds: async () => new Map() } as never, memories: { listBatches: async () => [{ batchStartFloor: 1, batchEndFloor: 30, stale: true }] , markAllBatchesStale: async () => {} } as never, generator: { generate: async () => { staleRegenerated += 1; return []; } } as never });
